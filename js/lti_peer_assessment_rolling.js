		...
		$("style").append('.focusUser { background-color: lightgreen; } .okmsg {color: #00748B;}');

		$("button#newAssessment").click(function() {
			$("form#new").submit();
		});
		...
		$('.student_assess').keyup(function () {
            ...
            var score = <?= $js_vars["score"] ?>; // jshint ignore:line

			if(this.value > score) {		// jshint ignore:line
				this.value = score;
			}
			if(this.value < 0) {
				this.value = 0;
		...
		$("button#assess").click(function(e) {
			...
			if(formok === true) {
				confirmed = bootbox.confirm("Once you have submitted you will not\
				 									be able to return to this form to amend\
													your marks for this assessment.\
													\nYou can, however, return later\
